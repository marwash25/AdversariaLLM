...
1) In memory:
    ...
    plot_precomputed_cross(cross_normalized, raw=raw, out_path="cross.png", style="heatmap")

2) From ``alpha_zero_bound(..., debug_save_cross="bounds.pt")``:
...
def _load_precomputed(path: Path) -> tuple[Optional[Tensor], Optional[Tensor], dict[str, Any]]:
    """Return (primary tensor for plotting normalized cross, raw, meta)."""
    meta: dict[str, Any] = {}
    ...
    if isinstance(obj, dict):
        for key in ("n", "b", "nb"):
            ...
            if isinstance(v, int):
                meta[key] = v
        raw = obj.get("raw")
        raw = raw if isinstance(raw, Tensor) else None
        cn = obj.get("cross_normalized")
        cn = cn if isinstance(cn, Tensor) else None
        if cn is not None:
            return cn, raw, meta
        if raw is not None:
        ...
        for v in obj.values():
            if isinstance(v, Tensor):
                return v, None, meta
    raise TypeError(f"Unsupported content in {path}")
...
def main() -> None:
    ...
    p.add_argument("--precomputed-pt", type=Path, default=None, help="torch.save dict/tensor or .npy")
    ...
    if args.precomputed_pt is not None:
        cross, raw, meta = _load_precomputed(args.precomputed_pt)
